...
import { ME, USERS, CHAT_CHANNELS, TOPICS, FORUM_CATEGORIES, VAULT_ITEMS, toast } from './data.jsx';
import { Icons, BrandMark, Avatar, Ladrilho } from './components.jsx';

export const NAV = [
  { group: 'PRINCIPAL', items: [
    { id: 'dashboard', label: 'Dashboard', icon: 'home' },
    { id: 'chat', label: 'Chat', icon: 'chat', count: 23 },
    { id: 'forum', label: 'Fórum', icon: 'forum', count: 7 },
...
export function Sidebar({ current, setCurrent, onOpenSearch }) {
  ...
  return (
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav">
        ...
        <div style={{ marginTop: 'auto', padding: '16px 10px 8px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}><strong style={{ color: 'var(--text-primary)' }}>{onlineCount}</strong> membros conectados</span>
          </div>
          <div style={{ display: 'flex', marginLeft: 4 }}>
            {USERS.filter(u => u.status === 'online').slice(0, 6).map((u, i) => (
      ...
      <div className="sidebar-foot">
        ...
        <button className="icon-btn tip" data-tip="Sair" onClick={() => toast('Até logo, ' + ME.name.split(' ')[0] + '!', 'info')}>
          {Icons.logout}
        </button>
      </div>
    </aside>
...
export function Topbar({ crumbs, children, onThemeToggle }) {
  return (
    <header className="topbar">
      ...
      <div className="topbar-actions">
        <button className="icon-btn tip" data-tip="Alternar tema" onClick={onThemeToggle}>
          {Icons.moon}
        </button>
        <button className="icon-btn tip" data-tip="Notificações">
          {Icons.bell}
          <span className="notif-dot" />
        </button>
        <button className="icon-btn tip" data-tip="Configurações">{Icons.settings}</button>
...
export function CmdK({ open, onClose, setCurrent }) {
  ...
  const results = useMemo(() => {
    ...
    USERS.forEach(u => {
      if (!Q) return;
      if (u.name.toLowerCase().includes(Q) || u.handle.includes(Q) || u.role.toLowerCase().includes(Q))
        out.push({ type: 'Pessoa', label: u.name, sub: u.role, user: u, action: () => { setCurrent('profile'); onClose(); toast('Perfil de ' + u.name, 'info'); } });
    });
    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border-default)', borderRadius: 14, boxShadow: 'var(--shadow-lg)', width: '100%', maxWidth: 620, overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ maxHeight: 380, overflow: 'auto', padding: 6 }}>
          {!q && <div style={{ padding: '24px 18px', color: 'var(--text-muted)', fontSize: 13 }}>
            Digite pra buscar. Experimente <span className="mono" style={{ background: 'var(--bg-elev-1)', padding: '2px 6px', borderRadius: 4 }}>vps</span> ou <span className="mono" style={{ background: 'var(--bg-elev-1)', padding: '2px 6px', borderRadius: 4 }}>rafael</span>.
          </div>}
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <div style={{ height: 110, background: `linear-gradient(135deg, ${user.color}, ${user.color}99)`, position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '0 var(--s-6) var(--s-6)', marginTop: -40 }}>
          ...
          <div style={{ marginTop: 14 }}>
            ...
            <div style={{ color: 'var(--text-secondary)', fontSize: 14, marginTop: 4 }}>{user.role} · @{user.handle}</div>
          </div>
          <p style={{ color: 'var(--text-secondary)', marginTop: 14, marginBottom: 20 }}>{user.bio}</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, padding: '16px 0', borderTop: '1px solid var(--border-subtle)', borderBottom: '1px solid var(--border-subtle)' }}>
            <ModalStat label="Posts" value={user.posts} />
            <ModalStat label="Reputação" value={user.reputation} />
            <ModalStat label="Membro desde" value={user.joined} />
          </div>
          <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
            <button className="btn btn-primary" style={{ flex: 1 }} onClick={() => { toast('Mensagem iniciada com ' + user.name, 'ok'); onClose(); }}>Enviar mensagem</button>
